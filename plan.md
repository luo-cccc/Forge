...
当前状态：进行中。`writer_agent/kernel.rs` 已有 `kernel_chapters.rs`、`kernel_helpers.rs`、`kernel_ops.rs`、`kernel_prompts.rs`、`kernel_review.rs` 等辅助模块；`writer_agent/kernel_task_packet.rs` 已承接 TaskPacket 构建、context budget trace 和 trace state expiry helper，并保持 `writer_agent::kernel::build_task_packet_for_observation` 对外路径稳定。`writer_agent/kernel_metrics.rs` 已承接 `WriterProductMetrics` 和 trace-derived product metrics 计算，并保持 `writer_agent::kernel::WriterProductMetrics` 对外路径稳定。`kernel.rs` 当前约 4626 行，下一步继续拆 run-loop / proposal / feedback / memory candidate 责任。
...
11. 拆 `agent-evals/src/evals.rs`。
12. 保持 public protocol 稳定。
