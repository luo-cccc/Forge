...
- manual ask converted-to-operation rate 进入 Inspect Run Health，并由 `writer_agent:product_metrics_manual_ask_conversion` 覆盖。（已完成）
...
2. Inspector timeline。
   ...
   - Inspect 模式右侧摘要已覆盖 provider budget、latest failure、latest save、latest receipt、latest artifact、save-to-feedback latency、post-write diagnostics、proposal context budgets 和 context source trends。
