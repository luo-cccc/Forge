...
- 当前本轮验证基线由 `scripts/verification-baseline.cjs` 维护，`agent-harness-core` 为 88 tests passing，`agent-writer` 为 227 tests passing，完整 `agent-evals` 为 246/246 passing；`check:baseline` 现在除同步 README / project-status 外，还会解析真实 `cargo test -- --list`、`cargo run -p agent-evals`、command audit 和 architecture guard 输出，避免只验证文档等于脚本。新增真实 provider smoke tests 已接入 `src-tauri/src/api_integration_tests.rs`，但必须显式设置 `FORGE_REAL_API_TESTS=1` 且提供真实 `OPENAI_API_KEY` 才会执行，普通验证保持离线；真实链路现在不只有单功能 smoke，还新增了 `api_integration_tests::real_author_session_three_chapter_smoke` 作为 3 章长会话回归门。`llm_runtime` 已把 general chat / JSON / 章节草稿 / ghost preview / 分析 / 平行草稿 / 手动改写 / 工具续写 / Project Brain stream 的 temperature、max_tokens 和 provider-scoped reasoning 开关拆成受限环境参数，支持真实环境按功能低成本调参；这些默认值现已与 Node 长会话 runner 共享同一份 `config/llm-request-profiles.json`。2026-05-05 本地真实 provider 长会话校准使用 OpenRouter `deepseek/deepseek-v4-flash` 跑 5 章“镜中墟”作者会话：短结构化 profile 禁用 reasoning 后每轮 35 次操作 0 provider failures，JSON 有效率 1.0，A/B/C 分支有效率 1.0，hook rate 1.0，embedding 维度 1536。章节 reasoning A/B 的证据是：开启章节 reasoning 时平均 chat latency 约 6.9s、p95 约 25.0s、最低锚点命中率 0.8；后续把章节默认改成 disable reasoning、修正 runner 默认值并接入锚点承载评分后，当前低延迟章节 profile 的最佳实测已进一步收敛到 `chapter_draft.maxTokens=640`，最新 5 章实测达到平均 chat latency 约 4.5s、p95 约 12.5s、最低锚点命中率 0.8、最低锚点承载率 0.8，且 runner 不再报 findings。针对显式按需工具的后续校准也已完成第一轮：`analysis` 和 `parallel_draft` 不需要再做额外触发门槛，它们本来就是显式 command；基于真实 provider 定点探针，当前共享默认值已收敛为 `analysis.maxTokens=384`、`parallel_draft.maxTokens=512`。同时已新增 `npm run probe:chapter-stability`，把第 3/4 章输入样本固化为 repeated-runs probe：在完全相同输入下，`chapter3` 的 `latencyStddev≈6.1s`、`charsStddev≈97`、`anchorCarryRate 0.6..1.0`，说明 provider 抖动和 prompt 不稳同时存在；`chapter4` 的 `latencyStddev≈7.1s`，但 `charsStddev≈36`、`anchorCarryRate 0.8..1.0`，说明更偏向 provider jitter 主导。进一步看生成预览，`chapter3` 的同输入输出会在“快速入门见倒影”和“铺垫旧债/账册/封门背景”之间摆动，而 `chapter4` 没有这么大的叙事漂移。现在 chapter3 的 product path 已按这个结论收敛：前文输入从自由摘要改成结构化 continuity block，当前章节 beat 自动拆成 `Primary objective / Immediate pressure / Required payoff / Hold-back reveal` 四段，避免把“进入镜中墟 / 见旧友倒影 / 意识到封门真相”一次塞给同一章。重跑 repeated-runs probe 后，`chapter3` 收敛到 `latencyStddev≈1.4s`、`charsStddev≈70`，明显优于之前的 `6.1s / 97`；而 `chapter4` 仍有 `latencyStddev≈15.0s` 但内容波动较小，进一步证明 chapter4 主要是 provider jitter，chapter3 确实有 prompt/context instability 成分。第一版 `writer_agent:anchor_carry_metric` 已落地，用来识别“锚点全被提到但没有通过行动、对话、后果或 payoff pressure 真正承载”的失败模式。这不是最终最优，后续仍需继续用真实作者项目校准锚点召回、锚点承载质量和延迟尾部。Story Contract 质量门槛已进入 generation/rewrite/diagnosis task packet：低质量合同会在可回放 TaskPacket trace 中产生 `story_contract_quality_gate` belief 和 `StoryContractQuality` required context。元认知硬门禁 eval 覆盖写作 run blocking、正文写入 operation blocking、恢复性 mission calibration operation 放行、以及专用恢复 run 保持只读任务边界；`WriterMetacognitiveSnapshot` 会从 context pressure、failure bundle、post-write diagnostics、低置信 proposal、重复忽略率和 durable-save 健康度聚合风险等级与建议动作，并在 Inspector / trajectory 展示 `writer.metacognition`。Inspector metacognition 卡片已补恢复 CTA，可跳转 Review、诊断/保存、失败、上下文和 meta 视图，也可通过 `run_metacognitive_recovery` 触发只读 Planning Review / Continuity Diagnostic 恢复 run。
