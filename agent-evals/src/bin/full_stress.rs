...
fn main() {
    ...
    let chapter_count: usize = std::env::var("FORGE_STRESS_CHAPTER_COUNT")
        .unwrap_or_else(|_| "10".into())
        .parse()
        .unwrap_or(10);
    let api_timeout: u64 = std::env::var("FORGE_STRESS_TIMEOUT_SECS")
        .unwrap_or_else(|_| "180".into())
        .parse()
        .unwrap_or(180);

    eprintln!(
        "Forge Full Stress — {} chapters, {} @ {}",
        chapter_count, model, api_base
    );

    let memory = WriterMemory::open(Path::new(":memory:")).unwrap();
    let pid = "stress";
    memory
        .ensure_story_contract_seed(pid, "test", "fantasy", "quest", "redemption", "")
        .unwrap();
    memory
        .upsert_character(
            "林墨",
        ...
        .unwrap();
    memory
        .upsert_character("张三", &["三哥".to_string()], "supporting", "林墨同伴")
        .unwrap();
    memory
        .upsert_character("李四", &[], "supporting", "北境商队幸存者")
        .unwrap();
    memory
    ...
    for i in 1..=chapter_count {
        ...
        let outcome = if generated.is_empty() {
            "failed"
        } else if chars >= 3000 && chars <= 4000 {
            "valid"
        } else if chars < 3000 {
            "under"
        } else {
        ...
        if generated.is_empty() {
            ...
            let delta = build_basic_chapter_settlement_delta(
                pid,
                &title,
                "a001",
                &generated,
            ...
            if !setl_ok {
                errors.push("settlement failed".into());
            }
            ...
            eprintln!(
                ...
                chars, outcome, latency, setl_ok, cc, pc, kc
            );

            // Incremental report
            report.avg_final_chars = report
                .chapters
                .iter()
                .map(|c| c.final_chars as f64)
                .sum::<f64>()
                / report.completed as f64;
            report.compliance_rate = report
                .chapters
                .iter()
                .filter(|c| c.outcome == "valid")
                .count() as f64
                / report.completed as f64;
            let mut s: Vec<u64> = latencies.clone();
            s.sort();
            report.p95_latency_ms = s
                .get((s.len() as f64 * 0.95) as usize)
                .copied()
                .unwrap_or(0);
            report.avg_latency_ms = latencies.iter().sum::<u64>() as f64 / latencies.len() as f64;
            report.min_final_chars = report
                .chapters
                .iter()
                .map(|c| c.final_chars)
                .min()
                .unwrap_or(0);
            report.max_final_chars = report
                .chapters
                .iter()
                .map(|c| c.final_chars)
                .max()
                .unwrap_or(0);
            if let Some(l) = report.chapters.last() {
                report.entity_stats_after = format!(
                    "{}c/{}p/{}k",
                    l.character_count, l.promise_count, l.knowledge_count
                );
            }
