...
use agent_writer_lib::writer_agent::promise_planner::{
    knowledge_readiness_factor, promise_subject_pressure, timeline_due_factor,
};
use std::path::Path;
...
pub fn run_planner_fallback_eval() -> EvalResult {
    ...
    if !fallback_works {
        errors.push(format!(
            "fallback failed: pressure={} kr={} td={}",
            base_pressure, kr, td
        ));
    }
    EvalResult {
        ...
        actual: format!(
            "fallbackOk={} pressure={} kr={} td={}",
            fallback_works, base_pressure, kr, td
        ),
        errors,
    }
}
