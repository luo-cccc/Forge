...
pub fn run_input_compiler_eval() -> EvalResult {
    ...
    memory.upsert_character("\u{6797}\u{58a8}", &[], "protagonist", "\u{4e3b}\u{89d2}").unwrap();
    memory.upsert_knowledge_item("setting_valley", "objective", "test").unwrap();
    ...
    EvalResult::pass_if("input_compiler", ok, format!("compiledOk={} evidenceCount={}", ok, compiled.selected_evidence.len()))
}
