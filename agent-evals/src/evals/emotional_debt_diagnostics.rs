...
pub fn run_emotional_debt_diagnostics_eval() -> EvalResult {
    ...
    let paragraph_with_resolution = "林墨深吸一口气，决定放下过去的愤怒。他原谅了那个曾背叛他的人。";
    let results_resolved = engine.diagnose(
        paragraph_with_resolution,
        0,
        "Chapter-3",
        "eval",
        &memory,
    ...
    if !emotional_resolved.is_empty() {
        errors.push(format!(
            ...
            emotional_resolved.iter().map(|r| &r.message).collect::<Vec<_>>()
        ));
    }
