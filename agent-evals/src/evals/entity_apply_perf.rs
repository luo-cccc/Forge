...
pub fn run_entity_apply_perf_eval() -> EvalResult {
    ...
    memory.upsert_character("target", &[], "protagonist", "the one").unwrap();
    for i in 0..100 { let _ = memory.upsert_character(&format!("filler_{}", i), &[], "supporting", ""); }
    ...
    EvalResult::pass_if("entity_apply_perf", state.is_some(), "entityScopedApply=true".to_string())
}
