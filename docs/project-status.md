...
- Writer Agent context relevance ranking now prioritizes Canon / Promise ledger slices by mission, next beat, result feedback, recent decisions, cursor-local story signals, open promises, and lightweight scene-type matches, with `WHY writing_relevance` explanations on retrieved entries. Project Brain / vector chunks are now reranked after semantic retrieval using the same writing-focus and scene-type signals.
...
- `cargo run -p agent-evals`: 93/93 passing
...
- P2.3 context relevance now covers Writer Agent ledger context for Canon / Promise slices plus Project Brain / vector chunk rerank, with eval coverage for ordinary semantic-similarity distractors and scene-type relevance signals. The remaining retrieval gap is real long-session Project Brain fixtures and injecting active WriterMemory focus into standalone `query_project_brain`.
