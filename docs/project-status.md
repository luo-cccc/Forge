...
- Writer Agent context relevance ranking now prioritizes Canon / Promise ledger slices by mission, next beat, result feedback, recent decisions, cursor-local story signals, open promises, and lightweight scene-type matches, with `WHY writing_relevance` explanations on retrieved entries. Project Brain / vector chunks are now reranked after semantic retrieval using the same writing-focus and scene-type signals; standalone `query_project_brain` also injects active WriterMemory focus into retrieval and rerank.
...
- `cargo run -p agent-evals`: 94/94 passing
...
- P2.3 context relevance now covers Writer Agent ledger context for Canon / Promise slices plus Project Brain / vector chunk rerank, including standalone `query_project_brain` WriterMemory focus injection. Eval coverage now includes ordinary semantic-similarity distractors, scene-type relevance signals, and active-mission focus overriding a surface-similar query. The remaining retrieval gap is real long-session Project Brain fixtures that validate candidate-pool recall and noise suppression over many chapters.
