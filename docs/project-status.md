...
- Canon / Promise memory candidate quality gates now run on the real local-save and LLM proposal paths: vague candidates are rejected, duplicates are deduped before writes, and conflicting canon candidates become explicit continuity review proposals instead of direct long-term memory writes. Style preference writes now reject vague, duplicate, and same-key conflicting entries before they can pollute the style ledger.
...
- `cargo test -p agent-writer`: 166 passing
- `cargo test -p agent-harness-core`: 79 passing
- `cargo run -p agent-evals`: 90/90 passing
...
- P2.2 memory-write gates now cover Canon / Promise proposal creation, foundation quality guards, and basic Style memory validation; the remaining memory quality gaps are safer same-entity attribute merges and richer style taxonomy.
