...
- P4 has started with an append-only `WriterRunEventStore`: observation, context-pack-built, task-packet, proposal, operation-lifecycle, feedback, and failure events are recorded with monotonic sequence numbers, persisted in WriterMemory SQLite, and exported through trajectory JSONL as `writer.run_event`.
...
- `cargo run -p agent-evals`: 132/132 passing
